<template>
  <td class="table-cell" :class="[customClass]" :colspan="widget.options.colspan || 1"
    ...
    <template v-for="(subWidget, swIdx) in widget.widgetList">
      ...
      <template v-else>
        <component :is="getComponentByContainer(subWidget)" :field="subWidget" :key="swIdx" :parent-list="widget.widgetList"
          :index-of-parent-list="swIdx" :parent-widget="widget">
          <!-- 递归传递插槽！！！ -->
          <template v-for="slot in Object.keys($slots)" v-slot:[slot]="scope">
            <slot :name="slot" v-bind="scope" />
