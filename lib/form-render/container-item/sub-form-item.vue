    ...
    <div :key="widget.id" class="sub-form-container" v-show="!widget.options.hidden">
      <el-row class="header-row">
        <div class="action-header-column">
          ...
          <el-button :disabled="actionDisabled" round type="primary" size="small" class="action-button"
          ...
          <div class="field-header-column"
            ...
            <span v-if="!!subWidget.options.labelIconClass" class="custom-label">
              <template v-if="subWidget.options.labelIconPosition === 'front'">
                ...
                <template v-else>
                  <svg-icon :icon-class="subWidget.options.labelIconClass" />{{ subWidget.options.label }}
                </template>
              </template>
        ...
        <template v-for="(subWidget, swIdx) in widget.widgetList" :key="subWidget.id + 'tc' + subFormRowId">
          <div class="sub-form-table-column hide-label" :style="{ width: subWidget.options.columnWidth }">
            <component :is="subWidget.type + '-widget'" :field="fieldSchemaData[sfrIdx][swIdx]"
              :key="fieldSchemaData[sfrIdx][swIdx].id" :parent-list="widget.widgetList" :index-of-parent-list="swIdx"
              :parent-widget="widget" :sub-form-row-id="subFormRowId" :sub-form-row-index="sfrIdx"
